...
let order = function(x,y) {
	if (x.frequency == 100)
	{
		return d3.ascending(x.winRatio, 0)
	}
	if (y.winRatio == 100)
	{
		return d3.ascending(0, y.winRatio)
	}
	return d3.ascending(x.winRatio, y.winRatio)
};

let order2 = function(x,y) {
	if (x.frequency == 100)
	{
		return d3.descending(x.winRatio, 0)
	}
	if (y.frequency == 100)
	{
		return d3.descending(0, y.winRatio)
	}
	return d3.descending(x.winRatio, y.winRatio)
};
...
/*
d3.csv("ChampsToPlotOnline.csv", function(data) {
	console.log(data);
	championData = data;
	...
	chart = BarChart(alphabet, {
	  ...
	  update = chart.update(d3.sort(alphabet, order2));
	myChart = chart;
});
*/
d3.csv("ChampsToPlotOnline.csv").then(data =>{
	championData = data;
	plotData = [];
	plotData = championData.filter(chooseTheChampion);
	plotData = plotData.filter(chooseTheLane);			
